...
function getOMDBResults(title) {
  ...
  if (title.length == 0) {
    ...
    //if a search term is entered...start the search process
    //build the URL string to execute the HTTP GET request using $.get
    $.get(OMDB_API_URL + "t=" + title, function(searchResult) {
      ...
      if (searchResult.Response == "True") { //if the search does return results, display each field and a poster image
